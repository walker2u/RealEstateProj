    ...
    const storeImages = (file) => {
        return new Promise((resolve, reject) => {
            ...
            uplaodTask.on("state_changed",
                ...
                () => {
                    getDownloadURL(uplaodTask.snapshot.ref).then((downlaodUrl) => {
                        resolve(downlaodUrl);
                    })
                }
            )
                    ...
                    <p className='text-red-700'>{imageUploadError && imageUploadError}</p>
                    {
                        formData.imageUrls.length > 0 && formData.imageUrls.map((url, index) => {
                            return (<div key={url} className='flex justify-between p-3 border items-center'>
                                ...
                                <button onClick={() => handleDeleteImage(index)} className='text-red-500 p-3 rounded-lg uppercase hover:opacity-95 bg-red-50' type='button'>Delete</button>
                            </div>)
                        })
                    }
                    <button
